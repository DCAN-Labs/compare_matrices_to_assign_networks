function [structure_percentC, structure_percentD] = network_alluvial(DscalarC,DscalarD,plot_subcortex,output_name,wb_command)
...
if skip_plotting ==0
    if sort_matrix ==0 %
        ...
        h = alluvialflow_change_colors(sorted_alluvial_b, sorted_labels_a, sorted_labels_b, string('Network Change'),1,Eidx, Fidx, isanet_in_current_setC, isanet_in_current_setD,output_name);
    end
end
        ...
        if k>2
            ...
            if sort_matrix ==0 %
                ...
                switch subcort_plot_type
                    case 'alluvial'
                        h= alluvialflow_change_colors(alluvial_matrix, left_labels, right_labels, structure_name,0,[],[], isanet_in_current_setC, isanet_in_current_setD,output_name); % unsorted
                    case 'donut'
                        num_greys_in_nets_a = sum(alluvial_matrix,2);
                        %                        [E,Eidx] = sort(num_greys_in_nets_a,'descend');
                        num_greys_in_nets_b = sum(alluvial_matrix,1);
                        ...
                        network_donut(dough,left_labels, right_labels, structure_name,0,Eidx, Fidx, isanet_in_current_setC, isanet_in_current_setD,skip_plotting,add_label_percentages,donut_label_font_size);
                ...
                switch subcort_plot_type
                    case 'alluvial'
                        h = alluvialflow_change_colors(sorted_alluvial_b, sorted_labels_a, sorted_labels_b, structure_name,1,Eidx, Fidx, isanet_in_current_setC, isanet_in_current_setD,output_name);
                    case 'donut'
                        dough{1,1} = num_greys_in_nets_a';
                        dough{2,1} = num_greys_in_nets_b';
                        network_donut(dough,left_labels, right_labels, structure_name,1,Eidx, Fidx, isanet_in_current_setC, isanet_in_current_setD,skip_plotting,add_label_percentages,donut_label_font_size);
                end
            end
        end %ifk>2
    end
    donut_handle.Position = [100 100 900 700]; %make a 800 x 600 figure.

     print('-dpng','-r300',donut_handle,[output_name '_donut_fig'])
end
